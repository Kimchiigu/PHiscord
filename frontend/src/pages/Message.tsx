...
const Message: React.FC<MessageProps> = ({
  ...
  const isFile = (url: string) =>
    /(\.xlsx|\.pdf|\.docx|\.doc|\.zip|\.rar)/.test(url);
  const isLink = (url: string) => /^(https?:\/\/[^\s$.?#].[^\s]*)$/i.test(url);
  ...
  const renderFilePreview = (url: string) => (
    <div className="mt-2 p-2 bg-gray-800 rounded-lg flex items-center">
      ...
      <div className="flex-1 overflow-hidden">
        <div className="text-blue-500 font-semibold truncate">
  ...
  return (
    <div
      className={`border-b border-gray-600 py-3 flex items-start mb-4 text-sm text-left group relative ${
        isMentioned ? "bg-indigo-700 rounded-xl" : ""
      }`}
    >
      <img
        src={src}
      ...
      <div className="flex-1 overflow-hidden">
        <div>
          <span className="font-bold text-[--display-color] cursor-pointer hover:underline mr-2">
            {user}
          </span>
          <span className="font-bold text-[--secondary-text-color] text-xs">
            {time}
          </span>
        </div>
        {isEditing ? (
          ...
          <>
            {messageType === "image" ? (
              ...
              <p
                className={`text-[--primary-text-color] leading-normal mt-2 text-[var(--font-size-${fontSize})]`}
              >
                {message}
              </p>
            )}
